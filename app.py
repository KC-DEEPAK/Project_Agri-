...
# Simple translation dictionary (you can expand this)
translations = {
    'en': {
        'login': 'Login',
        'register': 'Register',
        ...
        'welcome': 'Welcome',
        'language': 'Language',
        'crop_prediction_app': 'Crop Prediction App',
    ...
    'hi': {
        ...
        'logout_success': 'आप लॉग आउट कर चुके हैं।',
        'welcome': 'स्वागत है',
        'language': 'भाषा',
        'crop_prediction_app': 'फसल भविष्यवाणी ऐप',
    ...
    'te': {
        ...
        'registration_success': 'నమోదు విజయవంతమైనది! దయచేసి లాగిన్ అవ్వండి.',
        'passwords_not_match': 'పాస్వర్డ్లు సరిపోలడం లేదు!',
        'user_exists': 'వాడుకరి పేరు లేదా ఇమెయిల్ ఇప్పటికే ఉంది!',
        'invalid_credentials': 'చెల్లని వాడుకరి పేరు లేదా పాస్వర్డ్!',
        'logout_success': 'మీరు లాగ్అవుట్ అయ్యారు.',
        'welcome': 'స్వాగతం',
        'language': 'భాష',
        'crop_prediction_app': 'క్రాప్ ప్రిడిక్షన్ యాప్',
...
def set_language(language):
    if language in app.config['SUPPORTED_LANGUAGES']:
        ...
        flash(f'Language changed to {app.config["SUPPORTED_LANGUAGES"][language]}', 'success')
...
def index():
    if 'user_id' in session:
        context = {
            ...
            "_": get_translation  # Add translation function to context
        }
...
def register():
    if request.method == 'POST':
        ...
        if password != confirm_password:
            flash(get_translation('passwords_not_match'), 'error')
        ...
        try:
            ...
            conn.commit()
            conn.close()
            flash(get_translation('registration_success'), 'success')
            return redirect(url_for('login'))
        except sqlite3.IntegrityError:
            flash(get_translation('user_exists'), 'error')
            return render_template('register.html', 
                                 context={
                                     "supported_languages": app.config['SUPPORTED_LANGUAGES'],
                                     "current_language": session.get('language', 'en'),
...
def login():
    if request.method == 'POST':
        ...
        if user and check_password_hash(user[3], password):
            ...
            flash(get_translation('login_success'), 'success')
...
# Your existing routes (protected with login required)
...
# Your existing helper functions (keep them exactly as they are)
def TopFiveWinners():
...
if __name__ == "__main__":
    ...
    # Initialize commodities (your existing code)
